<?php
	...
	class CEMail
	{
		...
		{	
			...
			if($from_address == CConfig::OEI_FINANCE || $from_address == CConfig::OEI_SUPPORT)
			{
				$this->objConnection = $this->objMail->connect(CConfig::MC_MTA_ONE, CConfig::MC_PORT, $from_address, $from_password, CConfig::MC_ENC_METHOD);
			}
			else 
			{
				$this->objConnection = $this->objMail->connect(CConfig::MC_MTA_TWO, CConfig::MC_PORT, $from_address, $from_password, CConfig::MC_ENC_METHOD);
			}
		}
		...
		{
			$this->objMail->addto($to_ary[0]);
			
			for($i = 1; $i < count($to_ary); $i++)
			{
				$this->objMail->addcc($to_ary[$i]);	
			}
