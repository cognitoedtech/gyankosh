		...
		for($index = 0; $index < count($candidate_ary); $index++)
		{
			if(!empty($candidate_ary[$index]))
			{
				...
				if(empty($isTestSessionExists) && empty($isTestCompleted))
				{
					...
					$cand_email = $objDB->GetUserEmail($candidate_ary[$index]);
					$is_valid_email = $objDB->IsValidEmail($candidate_ary[$index]);
					if (!empty($cand_email) && $is_valid_email == 1) {
						$objMail->PrepAndSendEditTestScheduleMail($objDB->GetUserName($candidate_ary[$index]), $cand_email, $objDB->GetTestName($test_id), $tschd_id, date("F j, Y", strtotime($scheduled_test['scheduled_on'])), $scheduled_by);
					}
