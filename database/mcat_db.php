<?php
	...
	class CMcatDB
	{
		...
		public function GetQuestionDetails($qid)
		{
			...
			$row['subject'] = $this->GetSubjectName($row['subject_id']);
			$row['topic']   = $this->GetTopicName($row['topic_id']);
		...
		public function PopulateTests($owner_id, $time_zone)
		 {
			...
			while($row = mysql_fetch_array($result))
			{
				...
				$aryProductDetails = $this->GetPublishedProductDetails($row['test_id'], CConfig::PT_TEST);
				$aryPublishedInfo = json_decode($aryProductDetails["published_info"], TRUE);
				...
				if(empty($aryPublishedInfo["cost"]["inr"]) || empty($aryPublishedInfo["cost"]["usd"]))
				{
					...
					{
						echo "<td>".$inr_element.$usd_element."</td>";
					}
				}
				else if($aryPublishedInfo["cost"]["inr"] && $aryPublishedInfo["cost"]["usd"] <= 0)
				{
					echo "<td>Free !</td>";
				}
				else
				{
					echo "<td><i class='fa fa-inr' aria-hidden='true'></i> ".$aryPublishedInfo["cost"]["inr"]."<br/> <i class='fa fa-usd' aria-hidden='true'></i> ".$aryPublishedInfo["cost"]["usd"]."</td>";
				}
				
				$pub_start_date="";
				if(strcasecmp($aryProductDetails['pub_start_date'],'0000-00-00') != 0 && $row['is_published'] == 1)
				{
					$pub_start_date = new DateTime($aryProductDetails['pub_start_date']);
					$pub_start_date->setTimezone($dtzone);
					$pub_start_date = $pub_start_date->format("M j, Y");
					printf("<td id='%d_row_start_date'>%s</td>", $row['test_id'], $pub_start_date);
				}
				else 
				{
					echo "<td>Test is not live</td>";
					$pub_start_date = CConfig::CONST_NOT_APPLICABLE;
				}
				...
				if($row['is_published'] == 1)
				{
					if(strcasecmp($aryProductDetails['pub_end_date'],'0000-00-00') != 0)
					{
						...
						$pub_end_date = $pub_end_date->format("M j, Y");
						printf("<td id='%d_row_end_date'>%s</td>", $row['test_id'], $pub_end_date);
					}
					else
					{
						echo "<td>Never</td>";
						$pub_end_date = CConfig::CONST_NEVER;
					}
				}
				else
				{
					echo "<td>Not Appliable</td>";
					$pub_end_date = CConfig::CONST_NOT_APPLICABLE;
				}
				...
				echo "<input type='checkbox' made_publish='".$row['is_published']."' id='".$row['test_id']."_checkbox' class='publish' test_id='".$row['test_id']."' test_name='".$row['test_name']."' onclick='OnPublish(this);' ".$isChecked."></input>";
				echo "</div>";
				echo "<div class='col-lg-3 col-md-3 col-sm-3'>";
				echo "<button class='btn btn-success btn-xs' id='".$row['test_id']."_edit' test_id='".$row['test_id']."' test_name='".$row['test_name']."' onclick='OnEditTest(this);'".$isHidden."><i class='fa fa-pencil' aria-hidden='true'></i></button>";
				echo "</div>";
				echo "</div><br/>";
				echo "<div class='row'>";
				...
				echo "<span id='".$row['test_id']."_keywords'>".$keywords."</span>";
				echo "<span id='".$row['test_id']."_description'>".$description."</span>";
				echo "<span id='".$row['test_id']."_suggested_reads'>".(empty($aryPublishedInfo)?"":$aryPublishedInfo["suggested_reads"])."</span>";
				echo "<span id='".$row['test_id']."_who_should_buy'>".$aryPublishedInfo["who_should_buy"]."</span>";
		...
		public function GetProductImage($product_id, $product_type)
		{
			$query = sprintf("select product_image from published_products where product_id=%d and product_type=%d", $product_id, $product_type);
		
			$result = mysql_query($query, $this->db_link) or die('Get Product Image error: ' . mysql_error());
        ...
        public function GetPublishedProductDetails($product_id, $product_type)
        {
        	$retVal = null;
        	
        	$query 		= 	sprintf("select * from published_products where product_id=%d and product_type=%d", $product_id, $product_type);
        	$result		=	mysql_query($query, $this->db_link) or die('Get Published Product error : ' . mysql_error());
        ...
        public function UnPublishProduct($product_id, $product_type)
        {
        	$retVal = null;
		...
		public function StayConnected($strEmail)
		{
			$query  = sprintf("insert ignore into stay_connected(email) values('%s')", mysql_real_escape_string($strEmail));
			
			$result = mysql_query($query, $this->db_link) or die('StayConnected error: ' . mysql_error());

			return $result;
		}
		
		public function GetDynamicTestVisibility($test_id)
		{
			$retVal = 0;
        		
        	$query = sprintf("select visibility from test_dynamic where test_id='%s'", $test_id);
        	
        	$result = mysql_query($query, $this->db_link) or die('Get Dynamic Test Visibility error : ' . mysql_error());
        	
        	if(mysql_num_rows($result) > 0)
        	{
        		$row = mysql_fetch_array($result);
        		$retVal = $row['visibility'];
        	}
