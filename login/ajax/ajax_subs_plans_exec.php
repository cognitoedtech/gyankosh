<?php
	...
	if (!$resp->is_valid)
	{
		...
		$ajax_response = "<div style='color: red'>Dear User,<br/><br/> Your usage quotes request had been failed due to incorrect or empty captcha value, please re-fill the form.<BR/><BR/>-Team ".CConfig::SNC_SITE_NAME."</div>";
	}
	else if(empty($name))
	{
	...
	else 
	{
		...
		*/
		...
		// Request Message
		$form_details .= "<tr>";
		$form_details .= "<td><b>Message:</b></td>";
		$form_details .= "<td>".$message."</td>";
		$form_details .= "</tr>";
		...
		$objMail->PrepAndSendSubsPlanQuoteRequest($subject, $form_details);
		...
		$objMail->PrepAndSendSubsPlanQuoteRequestAck($name, $email, $subject, $ack_messsage);
