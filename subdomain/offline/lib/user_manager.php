		...
		*/
		public function VerifyUser($email, $password)
		{
			...
			if($val_pass != -1)
			{
				...
				{
					$user_type_val = $this->GetFieldValueByEmail($email, "user_type") ;
					if($user_type_val == CConfig::UT_INDIVIDAL && strcasecmp($password, $val_login_name) == 0)
					{
						$bResult = 2;
					}
				}
		...
		public function AddUser(CUser $objUser)
		{
			...
			$user_id = CUtils::uuid() ;
			$query = sprintf("INSERT INTO users(".CUser::FIELD_USER_ID.", ".CUser::FIELD_USER_TYPE.", ".CUser::FIELD_LOGIN_NAME.", ".CUser::FIELD_FIRST_NAME.", ".CUser::FIELD_LAST_NAME.", ".CUser::FIELD_PASSWORD.", ".CUser::FIELD_CONTACT_NO .", ".CUser::FIELD_EMAIL.", ".CUser::FIELD_GENDER.", ".CUser::FIELD_CITY.", ".CUser::FIELD_STATE.", ".CUser::FIELD_COUNTRY.", ".CUser::FIELD_DOB.") VALUES('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s') ;", 
							$user_id, $objUser->GetUserType(), $objUser->GetLoginName(), $objUser->GetFirstName(), $objUser->GetLastName(), md5($objUser->GetPassword()), $objUser->GetContactNo(), $objUser->GetEmail(), $objUser->GetGender(), $objUser->GetCity(), $objUser->GetState(), $objUser->GetCountry(), $objUser->GetDOB()) ;
        ...
        public function IsUsersTableEmpty()
        {
        	...
        	$result = mysql_query($query, $this->db_link_id) or die("Is Users Table Empty Error: ".mysql_error($this->db_link_id)) ;
